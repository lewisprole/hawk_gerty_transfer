...
#ifdef THREE_BODY_H2_NOMERGE
  if (SphP[i].Density > 1  && SphP[i].Density < 1e8) 
     return 0;
#endif
  ...
  if(P[i].Pos[WINDTUNNEL_COORD] < All.InjectionRegion)  // injection region
    {
      ...
      //    return 1;
          ...
          boxsizes[WINDTUNNEL_COORD] - P[i].Pos[WINDTUNNEL_COORD] > All.InjectionRegion)  // the "normal region"
    {
      if(All.MaxVolumeDiff > 0 && SphP[i].Volume > 0.3 * All.MaxVolumeDiff * SphP[i].MinNgbVolume)
        return 0;
    }
...
#endif
#else
  sound_speed = sqrt(BOLTZMANN*FIX_JEANS_TEMPERATURE / (2.4*PROTONMASS)) / All.UnitVelocity_in_cm_per_s;
#endif
